...
import static com.norconex.collector.core.store.impl.mongodb.MongoDataStore.idFilter;
...
public class MongoDataStoreEngine
    ...
    public boolean clean() {
        boolean hasCollections = database.listCollectionNames().first() != null;
        database.drop();
    ...
    public boolean renameStore(IDataStore<?> dataStore, String newName) {
        boolean targetExists = colExists(newName);
        MongoDataStore<?> mongoStore = (MongoDataStore<?>) dataStore;
        String oldName = mongoStore.rename(database.getName(), newName);
    ...
    public Optional<Class<?>> getStoreType(String name) {
        ...
        String type = (String) storeTypes.find(
